  ...
  return (
    ...
    >
      <div className="relative">
        ...
        <span className="absolute -bottom-1 -end-1 flex h-8 w-8 items-center justify-center rounded-full bg-accent text-white shadow-lg ring-4 ring-white dark:ring-slate-800">
          <Clock3 className="h-4 w-4" />
        </span>
      </div>
      <div>
        <h3 className="text-2xl font-extrabold text-slate-700 dark:text-slate-200">{t('locked.title')}</h3>
        <p className="mx-auto mt-1.5 max-w-sm text-sm text-slate-400 dark:text-slate-400">{t('locked.hint')}</p>
      </div>
    </div>
  )
}
