...
export function Dialog({ open, onClose, title, description, children, footer, size = 'md' }: DialogProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto p-4 sm:p-6">
      <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-[2px] animate-fade-in dark:bg-slate-950/60" onClick={onClose} />
      <div
        role="dialog"
        aria-modal="true"
        className={cn(
          'relative z-10 my-8 w-full rounded-xl bg-white shadow-2xl animate-scale-in dark:bg-slate-800',
          SIZES[size],
        )}
      >
        {(title || description) && (
        ...
        {footer && (
          <div className="flex items-center justify-end gap-2 border-t border-slate-100 px-6 py-3.5 dark:border-slate-700">
            {footer}
          </div>
        )}
      </div>
